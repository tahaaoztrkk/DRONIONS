...
# What the trained detector's box actually measures, as a multiple of the
# object's physical width.
#
# A single global factor was tried first and is wrong, because the ratio is a
# property of the object rather than of the model. Measured over 16 detections
# with segmentation-derived labels: the open laptop's footprint from above is
# 1.29 times its quoted width because the screen and base both show, a book
# 1.15, a mug 1.01, and a phone only 0.81 -- its 0.16 m is the long side, and
# seen at an angle from above it never presents all of it.
#
# This is a measurement of the mask, not a correction of an error. The earlier
# 1.18 was the latter: it covered labels that were uniformly too wide, and the
# segmentation labels removed that. What remains is that "how wide is a laptop"
# and "how wide does a laptop look from a drone" are different questions.
TRAINED_BOX_SCALE = {
    "laptop": 1.29,
    "book": 1.15,
    "mug": 1.01,
...
# For a class with no measurement of its own. Close to 1.0 because that is what
# an unmeasured object should be assumed to be, rather than the median of four
# objects that happen to have been measured.
TRAINED_BOX_SCALE_DEFAULT = 1.0
...
def range_from_apparent_size(candidate, target: str,
    ...
    if not width_m or not img_w:
        return None
    # The trained detector's box measures the object's silhouette, which is not
    # the same as its quoted width -- see TRAINED_BOX_SCALE. Per object rather
    # than one global number, because the two differ in opposite directions:
    # a laptop looks wider than it is quoted, a phone narrower.
    if getattr(candidate, "source", "") == "trained":
        width_m *= TRAINED_BOX_SCALE.get(target.lower().strip(),
                                         TRAINED_BOX_SCALE_DEFAULT)
