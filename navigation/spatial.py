...
# How much wider the trained detector's boxes run than the object really is.
# Measured, not assumed: 11 detections across four objects, 1.04 to 1.38, median
# 1.18. See range_from_apparent_size for why it exists and what it does not fix.
TRAINED_BOX_INFLATION = 1.18
...
def range_from_apparent_size(candidate, target: str,
    ...
    if not width_m or not img_w:
        return None
    # The trained detector draws systematically wider boxes than the table
    # describes, so its detections read as nearer than they are, and the
    # approach then announced arrival almost where it started.
    #
    # The cause is in the labels rather than the model: they were generated by
    # projecting each object's axis-aligned 3D extent, which is larger than its
    # silhouette, and the model learned to reproduce that. Measured across 11
    # detections the boxes run 1.04 to 1.38 times too wide, median 1.18.
    #
    # Correcting by the median removes the bias and leaves about 14% of spread,
    # which is the honest state of it: the real fix is tighter labels and a
    # retrain, and this is a correction applied to a known error rather than a
    # calibration of anything.
    if getattr(candidate, "source", "") == "trained":
        width_m *= TRAINED_BOX_INFLATION
